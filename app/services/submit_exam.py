...
async def submit_exam(
    ...
    # One row for every (question, option) pair in this exam. LEFT JOIN so
    # a question with no options at all still yields a row (with NULLs)
    # instead of silently dropping out of the exam. Ordered by
    # `(order, question_id)`, which keeps each question's rows contiguous and
    # in exam order, so the dicts below come out in that order too.
