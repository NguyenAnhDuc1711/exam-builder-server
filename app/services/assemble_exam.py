...
async def create_exam(
    ...
    """
    existing_ids = await QuestionRepository(session).get_existing_ids(question_ids)
    ...
    ExamRepository(session).add(exam)
    await session.commit()
    return exam


async def get_exam(session: AsyncSession, exam_id: int) -> Exam | None:
    """The exam with this id, with its question order eager-loaded."""
    return await ExamRepository(session).get_by_id_with_questions(exam_id)
